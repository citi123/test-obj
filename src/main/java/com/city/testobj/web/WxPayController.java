...
//		params.put("spbill_create_ip", getIpAddr(request));
        ...
        params.put("notify_url", "http://139.199.165.21:8080/test-obj-war/jsp/receive/notify");
        ...
        params.put("time_expire", LocalDateTime.now().plusMinutes(5 * 60).format(DateTimeFormatter.ofPattern("uuuuMMddHHmmss")));
        LOGGER.info("{}", params);
    ...
    public ApiResponse billQuery(String out_trade_no, String transaction_id) throws Exception {
        MyWXPayConfig config = MyWXPayConfig.getInstance();
        WXPay wxpay = new WXPay(config);
